...
int MedLDA::run_em(char* start, char* directory, Corpus* corpus, Params *param)
{
	...
	cout << "Inicializando modelo..." << endl;
	// initialize model
	SuffStats* ss = NULL;
	...
	//save_model(filename);TODO eu alterei
...
void MedLDA::save_model(char* model_root)
{
	...
	for (i = 0; i < m_nK; i++) {
		...
		for (j = 0; j < m_nNumTerms; j++) {
			if(m_dLogProbW[i][j] < -30){ //we only print values larger than 0.0000000001
				continue;
			}else{
				fprintf(fileptr, " %d:%5.3f", j, m_dLogProbW[i][j]);
			}
		}
		fprintf(fileptr, "\n");
	}
